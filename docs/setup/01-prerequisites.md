...
Prove that your workstation and cloud/SaaS access are ready before any repository foundation or Terraform apply. After this topic you can install CLIs to the pinned versions, authenticate to AWS, see the Route53 zone, have a GitLab project with a linked local Git repository, and know an SMTP mailbox exists for later Alertmanager tests.
